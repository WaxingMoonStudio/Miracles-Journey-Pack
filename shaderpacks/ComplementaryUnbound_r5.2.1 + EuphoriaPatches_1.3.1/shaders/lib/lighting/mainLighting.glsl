...
/*
Code is commented out, see reason at directional lightmap code, also this would break M1 support, function calling inside main()
...
void DoLighting(inout vec4 color, inout vec3 shadowMult, vec3 playerPos, vec3 viewPos, float lViewPos, vec3 geoNormal, vec3 normalM,
    ...
    // Shadows
        ...
        if (shadowMult.r > 0.00001) {
                ...
                vec3 worldPos = playerPos + cameraPosition;
                    ...
                    float unboundShadowMult = smoothstep1(pow2(min1(cloudSample * 0.2)));

                    shadowMult *= mix(unboundShadowMult, 1.0, NIGHT_CLOUD_UNBOUND_REMOVE * (1.0 - sunVisibility));
    ...
    vec3 blockLighting = lightmapXM * blocklightCol;
        ...
        float lViewPosL = length(playerPosLightM) + 6.0;
            ...
            #endif
            float dirHandLightmap = clamp01(dot(normalize(cameraHeldLightPos), ViewToPlayer(-normalM)) + 1.0);
            float dirHeldLightmapMix = mix(1.0, dirHandLightmap, 0.01 * max0(100.0 - pow2(lViewPos))); // this makes the directional held light go as far as the default one

            // mix with 0.25 to match directional blocklight and the strength of the default normals. Higher values have a bigger normal contrast
            heldLight = mix(heldLight, heldLight * dirHeldLightmapMix, 0.25);
            heldLight2 = mix(heldLight2, heldLight2 * dirHeldLightmapMix, 0.25);
        #endif
