            ...
            if (entityId < 50040) {
                ...
                if (entityId == 50040) { // Endermite
                    ...
                    if (entityColor.a < 0.001)
                        ...
                        #ifdef SOUL_SAND_VALLEY_OVERHAUL_INTERNAL
                            if (color.r > color.b * 2.0) color.rgb = changeColorFunction(color.rgb, 10.0, colorSoul, inSoulValley);
                        #endif
                        #ifdef PURPLE_END_FIRE_INTERNAL
                            if (color.r > color.b * 2.0) color.rgb = changeColorFunction(color.rgb, 10.0, colorEndBreath, 1.0);
                        #endif
                }
            }
        } else {
