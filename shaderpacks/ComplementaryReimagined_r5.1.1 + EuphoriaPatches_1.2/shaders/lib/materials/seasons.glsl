// Yes I hate myself for having done all of this in a big file and with functions. Thanks you me from a year ago
    ...
    vec3 oldColor = color.rgb;  // Needed for entities
        ...
        if (summerTime > 0) {
                ...
                if(isFoliage || mat == 10132 && glColor.b < 0.999) {
    ...
    float autumnOnlyForests = 1.0;
        ...
        if (autumnTime > 0) {
            autumnColor = mix(color.rgb, desaturatedColor, 0.65 * autumnOnlyForests);
                ...
                if(isFoliage) {
                    if(mat == 10007 || mat == 10008){ // Leaves except spruce and cherry
                    ...
                    if (mat == 10132 && glColor.b < 0.999) { // Grass Block grass part
                        ...
                        leafVariable *= (1.0 - pow(lmCoord.y, 30.0)) * pow(lmCoord.y, 2.0);
    ...
    #if (SEASONS == 1 || SEASONS == 3 || SEASONS == 4) && defined GBUFFERS_TERRAIN && LESS_LEAVES > 0
        if(mat == 10007 || mat == 10008){ // Leaves except spruce and cherry
        ...
        if (winterTime > 0) {
            ...
            float snowSide = 0.0;
                ...
                if(isFoliage) snowSide = mix(1.0, 0.0, 1 / (color.g * color.g) * 0.033); // make all foliage white
                    ...
                    if (mat == 10132){ // Grass Block
                        ...
                        for(int i = 1; i <= FLOWER_AMOUNT; i++){
                            if(NdotU > 0.5) {
                                ...
                                #if EMISSIVE_FLOWERS > 0
                                    float thisEmission = min(flower1Variable + flower2Variable + flower3Variable + flower4Emission + flower5Variable, 1.0);
                                    emission = max(emission, thisEmission);
                                #endif
                            }
                        }
                        #if EMISSIVE_FLOWERS > 0
